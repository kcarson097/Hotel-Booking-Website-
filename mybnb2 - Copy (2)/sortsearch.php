<?php
    ...
    if(isset($_POST['sortby'])){
       $sortby = $_POST['sortby'];
       $_SESSION['mybnbsortby'] = $sortby;
    }
    //print_r($_SESSION);

    //$city = $_SESSION['cityID'];
    //$guests = $_SESSION['mybnbguests'];
    //$sortby = $_SESSION['mybnbsortby'];
    ...
    //builds url string of array 
  function buildURLString($array,$arrayURLName,$endpoint){
    for($i=0;$i<count($array)-2;$i++){
      $endpoint .= $arrayURLName."=" . $array[$i] . "&";
    }
    $endpoint .= $arrayURLName."=" . $array[count($array)-1] ;
    return $endpoint;
  }
    

    //check if the sortby has come from a filter by page and call the appropriate endpoint !
    //check which session vars are set and call the appropriate api response
    if(isset($_SESSION['proptype'],$_SESSION['cancelpolicy'],$_SESSION['amenities'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
      $finalEndpoint = buildURLString($_SESSION['proptype'],"proptype[]",$endpoint);
      $finalEndpoint .= "&";
      $finalEndpoint = buildURLString($_SESSION['cancelpolicy'],"cancelpolicy[]",$finalEndpoint);
      $finalEndpoint .= "&";
      $finalEndpoint = buildURLString($_SESSION['amenities'],"amenities[]",$finalEndpoint);
      echo $finalEndpoint;
      $result = file_get_contents($finalEndpoint);
    ...
    else if(isset($_SESSION['proptype'],$_SESSION['cancelpolicy'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
    $finalEndpoint = buildURLString($_SESSION['proptype'],"proptype[]",$endpoint);
    $finalEndpoint .= "&";
    $finalEndpoint = buildURLString($_SESSION['cancelpolicy'],"cancelpolicy[]",$finalEndpoint);
    ...
    //all filters selected bar cancelpolicy
    else if(isset($_SESSION['proptype'],$_SESSION['amenities'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
    $finalEndpoint = buildURLString($_SESSION['proptype'],"proptype[]",$endpoint);
    $finalEndpoint .= "&";
    $finalEndpoint = buildURLString($_SESSION['amenities'],"amenities[]",$finalEndpoint);
    ...
    //all filters selected proptype
    else if(isset($_SESSION['cancelpolicy'],$_SESSION['amenities'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
    $finalEndpoint = buildURLString($_SESSION['cancelpolicy'],"cancelpolicy[]",$endpoint);
    $finalEndpoint .= "&";
    $finalEndpoint = buildURLString($_SESSION['amenities'],"amenities[]",$finalEndpoint);
    echo $finalEndpoint;
    $result = file_get_contents($finalEndpoint);
    ...
    //proptype 
    else if(isset($_SESSION['proptype'],$_SESSION['mybnbcity'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
    $finalEndpoint = buildURLString($_SESSION['proptype'],"proptype[]",$endpoint);
    echo $finalEndpoint;
    $result = file_get_contents($finalEndpoint);
    ...
    // cancelpolicy
    else if(isset($_SESSION['cancelpolicy'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
    $finalEndpoint = buildURLString($_SESSION['cancelpolicy'],"cancelpolicy[]",$endpoint);
    echo $finalEndpoint;
    $result = file_get_contents($finalEndpoint);
    ...
    //amenities
    else if(isset($_SESSION['amenities'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/filtering/sortfiltersearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}&";
      $finalEndpoint = buildURLString($_SESSION['amenities'],"amenities[]",$endpoint);
      echo $finalEndpoint;
      $result = file_get_contents($finalEndpoint);
      $data = json_decode($result,true);
      //this is needed to calculate number of pages to show for pagination
      $numberOfResults = $data["Number of results"];
    }else if(isset($_SESSION['mybnbcity'],$_SESSION['mybnbguests'])){
      $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/searches/sortsearch.php?location={$_SESSION['mybnbcity']}&accomodates={$_SESSION['mybnbguests']}&page={$page}&sortby={$_SESSION['mybnbsortby']}";
    ...
    // else{
    //   $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/searches/sortfiltersearch.php?page={$page}&sortby={$_SESSION['mybnbsortby']}&";
    //   $finalEndpoint = buildURLString($_SESSION['proptype'],"propertytype[]",$endpoint);
    //   echo $finalEndpoint;
    //   $result = file_get_contents($finalEndpoint);
    //   $data = json_decode($result,true);
    //   //this is needed to calculate number of pages to show for pagination
...
<div class="col-md-9">
          ...
          <div class="form-group searchpagebar needs-validated">
  ...
  <?php
        ...
        echo "<div class = row'>
              <div class = col-md-9>
                ...
                <form method = 'POST' action = 'sortsearch.php?page=$page'> 
                  <div class='btn-group sortbuttons' role='group' aria-label='Basic example'> 
                    ...
                    <button type='submit' value = 'Rating' name = 'sortby' class='btn btn-outline-primary'>Rating (Highest First)</button>
                    <button type='submit' value = 'Price' name = 'sortby' class='btn btn-outline-primary'>Price (Lowest First)</button>
                    <button type='submit' value = 'NumberOfLikes' name = 'sortby' class='btn btn-outline-primary'>Most Popular</button>
                  </div>
                  </form>
                  </div>
              </div>";
 ...
 <?php
    ...
    echo "
          <ul class='pagination d-flex justify-content-center'> 
              <li class='page-item'>
              <a class='page-link' href='sortsearch.php?page=1&rand=$num'>First</a>
              </li>";
      ...
      for($i=$startPage;$i<=$endpage;$i++){//generate links to page from start to end page
        echo "<li class='page-item'><a class='page-link' href='sortsearch.php?page=$i&rand=$num'>$i</a></li>";
      }
      echo "<a class='page-link' href='sortsearch.php?page=$numberpages&rand=$num'>Last</a>";
