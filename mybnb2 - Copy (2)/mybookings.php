<?php
    ...
    $endpoint = "http://kcarson09.lampt.eeecs.qub.ac.uk/mybnbapi/bookings/viewbookings.php?userID={$_SESSION['userID']}&page={$page}";
...
<section id = main>
      ...
      <div>
          <h3 class = "titlesindex"><?php echo"{$_SESSION['username']}"?>,Showing your Bookings : </h3>
      </div>
      <?php
      //check if there are items in array- the lentth will always be one as the api returns the number of results for the first index
        if(count($data)<=1){
          ...
          echo "<div><h3 class ='titlesindex'>No Bookings Found</h3><a href = 'index.php'>Return home</a></div>";
        }else{
              ...
              <div class='col-sm'>
                <div class='card h-100'>
                  ...
                  <div class='card-body'>
                    ...
                    <p class='card-text'>Price paid per night: <strong>$$price</strong></p>
                    <p class = 'card-text'>
                    ...
                    <form method = POST action = 'removebooking.php'>
                      <input type = 'hidden' name = 'prevurl' value = {$_SERVER['REQUEST_URI']}>
                      <input type = 'hidden' name = 'start' value = $start>
                      <input type = 'hidden' name = 'end' value = $end>
                      <button type = 'submit' class='btn btn-primary btn-md' name = 'prop' value = $PropertyID>Delete Booking</button>
                    </form>
                  </div>
                </div>
              </div>
...
<?php
    ...
    echo "
          <ul class='pagination d-flex justify-content-center'> 
              <li class='page-item'>
              <a class='page-link' href='mybookings.php?page=1&rand=$num'>First</a>
              </li>";
      ...
      for($i=$startPage;$i<=$endpage;$i++){//generate links to page from start to end page
        echo "<li class='page-item'><a class='page-link' href='mybookings.php?page=$i&rand=$num'>$i</a></li>";
      }
      echo "<a class='page-link' href='mybookings.php?page=$numberpages&rand=$num'>Last</a>";
